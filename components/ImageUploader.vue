...
const allPreviews = computed(() => {
  // If maxFiles is 1 and we have a new file, show ONLY the new file preview
  // This effectively "hides" the existing image to simulate replacement
  if (props.maxFiles === 1 && newFilePreviews.value.length > 0) {
    return newFilePreviews.value;
  }
...
function triggerImageFileInput() {
  ...
  // EXCEPTION: If maxFiles is 1, we allow clicking to "replace" even if 1 exists.
  if (props.maxFiles === 1) {
    imageFileInput.value?.click();
    return;
  }
  
  if (allPreviews.value.length >= props.maxFiles) return;
  imageFileInput.value?.click();
}
...
function handleImageFileChange(event: Event) {
  ...
  // Allow replacement if maxFiles is 1
  if (props.maxFiles !== 1 && currentTotal - newFiles.value.length + files.length > props.maxFiles) {
    toast({
  ...
  for (const file of Array.from(files)) {
    ...
    if (props.maxFiles === 1) break; // Only take the first one if limit is 1
  }
  ...
  <div class="w-full space-y-3">
    ...
    <div
      ...
      :class="[
        ...
        (maxFiles !== 1 && allPreviews.length >= maxFiles) || disabled ? '' : 'cursor-pointer',
      ]"
    ...
    >
      ...
      />
      <!-- Show dropzone UI only if no images are present OR if multi-upload is allowed (and we haven't hit limit) -->
      <!-- Adjusted logic: If maxFiles is 1 and we have a preview, we hide the dropzone text (unless we want to show it as 'click to replace'?) -->
      <!-- Current logic: Show if maxFiles > 1 OR (maxFiles=1 AND 0 previews) -->
      <div
